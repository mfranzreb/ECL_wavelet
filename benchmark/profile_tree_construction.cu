...
int main(int argc, char** argv) {
  ...
  if (alphabet_size < std::numeric_limits<uint8_t>::max()) {
    auto [alphabet, data] = ecl::generateRandomAlphabetAndData<uint8_t>(
    ...
    auto [alphabet, data] = ecl::generateRandomAlphabetAndData<uint16_t>(
        alphabet_size, data_size, true);
    ecl::WaveletTree<uint16_t> wt(data.data(), data_size, std::move(alphabet),
                                  0);
  } else {
    auto [alphabet, data] = ecl::generateRandomAlphabetAndData<uint32_t>(
        alphabet_size, data_size, true);
