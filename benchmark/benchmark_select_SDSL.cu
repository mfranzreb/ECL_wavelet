...
static void BM_SDSL(benchmark::State& state) {
  ...
  bool const sort_queries = state.range(1);
  auto const alphabet_size = state.range(2);
  auto const data_size = state.range(3);
  ...
  if (rebuild_wt) {
    ...
    std::tie(data, hist) = generateRandomDataAndHist<T>(alphabet, data_size);
    queries = generateRandomSelectQueries<T>(hist, num_queries, alphabet);
...
#pragma omp parallel for
      for (size_t i = 0; i < num_queries; ++i) {
        results_sdsl[i] =
            sdsl_byte_wt.select(queries[i].index_, queries[i].symbol_);
      }
    }
  } else {
    sdsl::wt_pc<sdsl::balanced_shape, sdsl::bit_vector, sdsl::rank_support_v5<>,
...
#pragma omp parallel for
      for (size_t i = 0; i < num_queries; ++i) {
        results_sdsl[i] =
            sdsl_short_wt.select(queries[i].index_, queries[i].symbol_);
      }
    }
  }
}
